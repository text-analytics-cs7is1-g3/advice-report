...
dfi = pd.read_csv(f"data/ds1.csv",chunksize=1024)
odf = pd.read_csv(f"data/ds1-you-thank.csv")
...
for df in dfi:
    ...
    #     "subname": df["subname"],
    # }, index=df.index)
    odf.loc[df.index, "c2_thankfulness"] = y_pred
    t1 = time.perf_counter()
    print(f"processed {len(df)} docs in {t1-t0:.0f} seconds")

odf.to_csv("data/ds1-you-thank-again.csv")
