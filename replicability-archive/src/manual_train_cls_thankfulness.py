...
import manual_bow
import features
# import polynomial_thankfulness as polynomial
import polynomial

folds = 50
X = polynomial.X_thankfulness_poly
y = features.y_thankfulness

print(X.shape, y.shape)
X_train, X_test, y_train, y_test = train_test_split(X.values, y, test_size=0.2, random_state=40, shuffle=True, stratify=y)
...
df.to_csv("cv_thankfulness.csv")

df_mf = pd.DataFrame(dummy_results_mf)
df_mf.to_csv("dummy_mf_thankfulness.csv")

import manual_train_cls_thankfulness_vis
