...
df = pd.read_csv("cv_thankfulness.csv")
# grouped_df = df.groupby('i')['accuracy'].mean()
...
df_mf = pd.read_csv("dummy_mf_thankfulness.csv")
...
plt.axhline(y=df_mf['accuracy_val'].mean(), label="dummy val", color="blue")
plt.axhline(y=df_mf['accuracy_val'].mean() + df_mf['accuracy_val'].std(), linestyle='--', lw=0.5, color="blue")
plt.axhline(y=df_mf['accuracy_val'].mean() - df_mf['accuracy_val'].std(), linestyle='--', lw=0.5, color="blue")
...
plt.xlabel('C')
plt.ylabel('Accuracy')
plt.title('Mean/Std Accuracy of $\\texttt{thankfulness}$' + f" classifier against C with {folds}-fold Cross-Validation")
plt.xscale('log')
plt.legend()
plt.savefig("fig/cv_thankfulness.pdf")
plt.show()
